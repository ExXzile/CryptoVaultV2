...
class VaultInit(Toplevel):
    def __init__(self):
        ...
        self.bottom_logo.grid(column=0, row=1, padx=(0, 12), pady=(30, 0), sticky="en")

        # entry boxes
        Label(
            self.bottom_frame, text="*Master Password:", font="Courier 8", bg="#f0eb4b"
        ).grid(column=0, row=0, padx=12, pady=(6, 0), sticky="w")
        self.master_key = Entry(
            self.bottom_frame, font="Terminal 10", width=36, show="✱"
        )
        self.master_key.focus_force()
        self.master_key.grid(column=0, row=0, padx=12, pady=(36, 0), sticky="w")

        Label(
            self.bottom_frame, text="*Master PIN:", font="Courier 8", bg="#f0eb4b"
        ).grid(column=0, row=2, padx=12, pady=(6, 0), sticky="w")
        self.master_pin = Entry(
            self.bottom_frame, font="Terminal 10", width=36, show="✱"
        )
        ...
        self.cancel_button.grid(
            column=0, row=3, columnspan=2, pady=(30, 12), sticky="e"
        )
        ...
        self.confirm_button = ttk.Button(
            self.bottom_frame, text="Confirm", width=8, command=_login
        )
        self.confirm_button.grid(
            column=0, row=3, columnspan=2, pady=(30, 12), padx=(0, 72), sticky="e"
        )
...
def vault_init():
    ...
    if _v_init.skeleton_key:
        _v_init.destroy()
        return _v_init.skeleton_key  # if encrypted data matches user entry, "skeleton_key" tuple
    else:
        return False
