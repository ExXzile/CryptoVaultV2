...
class MainApp:
    def __init__(self, master):
        ...
        self.find_button = ttk.Button(
            master, text="Find\nEntry", width=8, style="TButton", command=find_button
        )
        ...
        self.bottom_logo_canvas.grid(
            row=1, column=0, padx=(0, 186), pady=(3, 3), sticky="e"
        )
        ...
        self.new_button = ttk.Button(
            self.bottom_frame, text="New", width=8, style="TButton", command=new_entry
        )
        ...
        self.new_button = ttk.Button(
            self.bottom_frame, text="About", width=8, style="TButton", command=_about
        )
        self.new_button.grid(row=0, column=2, padx=48, pady=(0, 60), sticky="s")

        # quit button
        self.quit_button = ttk.Button(
            self.bottom_frame, text="Quit", width=8, style="TButton", command=sys.exit
        )
        ...
        if not Path("CryptoKey.dat").is_file():
            if not messagebox.askokcancel(
                "First Run", "User Master Key not Found\nCreate new?"
            ):
                sys.exit()
            else:
                sq_cur.execute(
