...
class VaultEntry(Toplevel):
    def __init__(self, skeleton_key, action, _id=0):
        ...
        # entry boxes
        Label(self.bottom_frame, text='*Master Account:', font='Courier 8', bg='#f0eb4b')\
        ...
        Label(self.bottom_frame, text='*Login Name:', font='Courier 8', bg='#f0eb4b')\
        ...
        Label(self.bottom_frame, text='*Password:', font='Courier 8', bg='#f0eb4b')\
        ...
        # notes text box
        Label(self.bottom_frame, text='Notes:', font='Courier 8', bg='#f0eb4b')\
            .grid(column=0, row=3, padx=12, pady=(12, 0), sticky='ws')
        self.notes_entry = Text(self.bottom_frame, width=20, height=8, wrap=WORD, font='Courier 8')
        ...
        self.generate_pass = ttk.Button(self.bottom_frame, text='Generate\nSecure',
        ...
        self.clip_copy = ttk.Button(self.bottom_frame,
                                    text='Copy to\nClipboard', width=9,
                                    style='TButton', command=_clip_copy)
        self.clip_copy.grid(column=0, row=2, padx=(84, 0), pady=(96, 0), sticky='w')

        # logo
        self.bottom_logo = PhotoImage(file='Icons\\CryptoVault_V2.png')
        self.bottom_logo_canvas = Canvas(self.bottom_frame, width=90, height=380,
        ...
        def _vault_it():
            ...
            if not all([master, login, password]):
                messagebox.showwarning('Missing Entries', 'please ensure all * fields are filled!', icon='warning')
                self.lift()
            else:
                vault_entry = Vault(self.key_hash, master, login, password, notes)
                query_data = [vault_entry.master, vault_entry.login, vault_entry.password, vault_entry.notes]
                query = ''
                ...
                if self.action == 'new':
                    ...
                    write_confirm = messagebox.askokcancel('Overwrite', 'Warning!\n'
                ...
                if write_confirm:
                    try:
                        ...
                        messagebox.showwarning('Error', 'Cannot Write to File!', icon='warning')
                    else:
                        sq_con.commit()
